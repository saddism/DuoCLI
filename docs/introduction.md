...
- **会话级草稿与可靠提交**：切换、刷新和重连不丢内容，同一提交不会因为重试重复执行
- **后台计划执行**：定时继续、多条命令和自动同意由主进程持久化调度
- **跨端快捷命令**：桌面和手机共享同一份命令，点击插入、确认后发送
- **全局多 Pane 工作区**：不同项目的终端可放在同一布局中并拖拽交换
- **手机工作台**：通知直达会话，横屏同时展示会话列表、终端与 Android 画面
- **文件与终端更好用**：中文/换行路径可点击，常见工作文件可预览或下载
- **Android 控制增强**：控制权切换、首击、长按、拖拽和系统导航键更可靠
- **CLI 支持扩展**：新增 DSH，并完善多家 CLI 的识别与恢复
