...
## v1.2.1 — 普通会话无限添加 & 多 Pane 体验优化 (2026-09-07)

> 一个窗口，随手管理你常用的 AI CLI。普通会话想开几个开几个；需要并行时，再把它们拆成清晰的分屏。

> 完整宣传版更新说明（中英双语）：[docs/updates/v1.2.1-release-notes.md](./docs/updates/v1.2.1-release-notes.md)

### 修复与改进

- **普通会话不再受 4 个分屏限制**：新建终端和侧边栏切换会复用当前焦点 Pane，继续创建的会话仍会保留在会话列表中。
- **分屏变成显式操作**：只有点击 Pane 的拆分按钮才会增加可见分屏；最多保留 4 个可读分屏，避免终端缩成不可用的小格子。
- **工作区更稳定**：修复启动时恢复历史 Pane 导致渲染器中断、终端无法创建的问题；旧的失效终端布局会在启动后自动清理。
- **AI 配置说明更清楚**：明确 AI 用于整理会话历史和自动生成对话标题，不参与终端命令执行。
- 新增会话切换回归测试；当前单元测试 **73/73** 通过。
